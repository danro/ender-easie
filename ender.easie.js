...
!function (win) {
	...
	, easie = function(p1x,p1y,p2x,p2y,name,forceUpdate) {
		...
		if ( !easings[name] || forceUpdate ) {
			...
			easings[name] = function(p, n, firstNum, diff) {
				return cubicBezierAtTimeLookup.call(null, p);
			}
			easings[name].params = [p1x,p1y,p2x,p2y];
		}
		return name;
	}
	...
	easie(0.000, 0.000, 1.000, 1.000, "linear");
	easie(0.250, 0.100, 0.250, 1.000, prefix);
